...
if( length(args) != 2 ) stop("Please provide two arguments (refpop and prefix)")
poprefname = args[1]
prefix = args[2]
pop <- read.table(poprefname, header = TRUE)[,c("IID","Population")]
train.data <- read.table("pca_proj_kg.sscore", header = TRUE, comment.char = "")[,c(2,5:14)]
colnames(train.data)[2:3] <- c("PC1","PC2")
train.phe <- merge(train.data, pop, by = "IID")
test.data <- read.table("pca_proj_kg_target.sscore", header = TRUE, comment.char = "")
colnames(test.data)[1] <- "FID"
target.cols <- grep("PC", colnames(test.data), value=TRUE)
test.data <- test.data[, c("FID","IID",target.cols)]
colnames(test.data)[3:4] <- c("PC1","PC2")
...
pred.pop <- predict(mymod,test.data[, c(3:12)], probability = TRUE)
